...
| Native thinking mode | ❌ Not available | ✅ Extended chain-of-thought reasoning |
...
**Native thinking mode.** Gemma 4 introduces extended chain-of-thought reasoning — the model produces explicit, traceable reasoning chains before committing to a response. For clinical decision support, this is clinically meaningful: it allows the specialist to follow and evaluate the model's reasoning process rather than receiving an opaque conclusion. MedGemma 1.5 4B, built on Gemma 3, does not have this capability regardless of fine-tuning.
